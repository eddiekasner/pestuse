...
title: |
  ...
  | Benton County, Washington 2014-2018
...
## Read 2014-2018 county-level estimates (Benton County, WA subset only)
...
Benton County ranks **#2 of Washington's 39 counties** by total
estimated agricultural pesticide mass applied (EPest-high method, summed
2014-2018), making it one of WA's five highest pesticide-use
counties. Benton County, part of the Tri-Cities area, is a center of Washington's wine industry (Yakima Valley and Columbia Valley AVAs), alongside tree fruit and irrigated row crops.
...
**Benton County, Washington** during `r recent_year`, as reported by
...
kable(display, "latex", booktabs = T, digits = 0, longtable = TRUE,
      caption = paste0("Top ", top_n, " EPest-high Estimates, ", county_name, ", WA, 1-Yr (", recent_year,
                        "), 3-Yr Avg, 5-Yr Avg, with Chemical Class (abbreviated; see the ",
                        "classification key table above)")) %>%
  kable_styling(latex_options = c("striped", "repeat_header", "HOLD_position"), font_size = 8)

write.csv(county_top, "benton.pesticide.use.2014-2018.csv", row.names = FALSE)
```

```{r top-chart, include=T, fig.cap="Top 25 compounds in Benton County by EPest-high mass applied, most recent year, colored by pesticide functional class."}
