...
wa_top <- wa_ranked %>%
  ...
  select(RANK_1YR, COMPOUND, CHEMICAL_CLASS, CHEM_ABBREV, FUNCTIONAL_CLASS, EPEST_HIGH_KG_1YR, EPEST_HIGH_KG_3YR_AVG, EPEST_HIGH_KG_5YR_AVG, YAKIMA_RANK_1YR)
```
...
Table \@ref(tab:class-key) and the full classification, with a separate
functional class column, in `data/pesticide_classification.csv`, shared
with the Washington pesticide class trends chapter), and — for comparison —
its `r recent_year` rank within Yakima County alone. Rankings and estimates
are
...
display <- wa_top %>% select(-CHEMICAL_CLASS, -FUNCTIONAL_CLASS)
